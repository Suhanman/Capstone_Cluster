...
AI 팀과 협업하여 관리자 앱에서 각각 K8s Job(데이터셋 취합),SageMakerTrainingJob(학습)을 수행하는 MLOps 파이프라인을 설계했습니다. 
TraginigJob은 프라이빗 서브넷에서 수행되어 외부로 노출될 수 있는 가능성을 막았으며 Spot 인스턴스 사용으로 비용을 절감했습니다.
또한 앞서 설계한 메시지 브로커와 SSE 허브 기반으로 TrainigJob의 상태를 로그 스트리밍으로 추적할 수있습니다.
이를 위한 Nat Gateway는 AWS의 서비스를 이용하는 것이 아닌 EC2의 kernel parameter tuning을 통해 필요 성능에 맞는 비용최적화를 하였습니다.
...
###  Issue 1. 하이브리드 클라우드 간 데이터 통신 문제해결
...
###  Issue 2. MLOps pipeline spec 저장소 선택
...
### 💸 Issue 3. AWS SageMaker 비용 최적화 — 스팟 인스턴스 전략
...
Kubernetes의 Docs를 처음부터 읽는 스터디활동을 진행하며 이러한 클러스터 구성 리소스들을 직접 수동으로 설치하고 배포하는 HardWay로
쿠버네티스 동작 원리에 대해서 조금 더 빨리 깊게 이해할 수 있었을 거라 생각이 듭니다.

### 프로젝트의 한계
본 프로젝트 서비스 인프라를 보다보면 몇가지 모순된점을 확인하실 수 있습니다.  
기본적으로 서비스 서버와 AI 추론 서버가 서로다른 토폴로지에서 돌아간다는 것 자체가 비효율적입니다.  
만약 실제 프로덕션이었다면 둘은 반드시 동일한 인프라 내에서 구동되어 latency를 최대한 줄이고 장애요소를 줄여야 할 것입니다.  
하지만 저는 서로다른 토폴로지의 서버끼리의 통신 구현과정에 대한 고민과 문제해결에 대한 궁금증이 더 컸었고, 오버엔지니어링을 시도하였습니다.  
만약 실제 프로덕션이었다면, AWS의 S3, Sagemaker 와 같은 클라우드를 활용하는 것이 효율적인 요소들만 온프레미스에서 끌어다 쓰는 방식으로
구축해보았을 것 같습니다.
...
| 항목                | 방향                                                          |
|-------------------|-------------------------------------------------------------|
| **AI 추론서버 이전**    | 하이브리드 IDC 구현을 완료하였으니 온프레미스 IDC로 마이그레이션해보는 작업 수행             |
| **스팟 인스턴스 중단 대응** | 체크포인트 기반 학습 재개 로직 보강으로 안정성 향상                               |
| **멀티 클러스터 연합**    | 온프레미스 + 클라우드 클러스터를 단일 평면으로 관리하는 Federation 검토(서버 이전하지 않을 시) |
